...
7. **Deploy to Vercel.** One new project per issue, so every topic gets its own permanent `*.vercel.app` URL and a new issue never overwrites a previous one: project name = `<brand.json vercel_project_prefix>-<slug>` (e.g. `buffalo-forge-newsletter-dirt-cheap-sword-forge`).
   - `deploy_to_vercel` **cannot create a new project itself** -- this account's token gets a 403 "You don't have permission to create a project" every time. Check `list_projects` first; if the project doesn't exist yet, ask the user to create an empty one via the Vercel dashboard with that exact name, in the same team, before deploying.
   - New projects Vercel creates via its dashboard default to the **Next.js** framework preset, which breaks a plain-HTML deploy (`NEXT_NO_VERSION` build error). Before deploying, check `get_project` for `framework: null` (matches the working `buffalo-forge-newsletter` project); if it instead shows `"nextjs"`, ask the user to change Settings -> General -> Framework Preset to **"Other"** and save, then re-check before deploying.
   - Once the project exists with `framework: null`, `mcp__claude_ai_Vercel__deploy_to_vercel` with `target: "production"` (safe here specifically because the project is brand-new -- there's nothing to overwrite), files = `ISSUE_DIR/web.html` renamed to `index.html` **only** -- no images to upload, since those are already hosted on GitHub. This still needs explicit user confirmation each time (the auto-mode classifier flags any `target: "production"` deploy regardless of project history) -- ask before deploying. Capture the returned URL.
...
- **Vercel: one project per issue, not one shared project.** The original design reused a single `buffalo-forge-newsletter` project across every issue -- simple, but every new topic's deploy silently replaced the previous one at the shared URL, so there was never more than one issue live at a time (older issues were only reachable by digging through Vercel's deployment history). The user explicitly asked to keep every issue permanently browsable instead, so each issue now gets its own project (`<vercel_project_prefix>-<slug>`), which means its own permanent default domain that nothing ever overwrites. The tradeoff: this creates one Vercel project per topic indefinitely (no cleanup step), and each is a brand-new project's first deploy -- so `target: "production"` is safe there specifically (nothing pre-existing to clobber), but still needs explicit per-deploy user confirmation since the classifier can't distinguish "new project" from "shared project" from the tool call alone.
- The very first newsletter issue (`rebar-to-knife-2026-07-09`) predates this per-issue-project convention and still lives in the shared `buffalo-forge-newsletter` project/URL from the original design. Leave it there -- its Gmail draft already points at that URL. Every issue from here on gets its own project.
